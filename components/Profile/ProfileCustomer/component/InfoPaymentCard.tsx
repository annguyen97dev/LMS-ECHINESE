...
const HistoryPayment = (props) => {
  ...
  const [studentPrice, setStudentPrice] = useState<ICourseOfStudentPirce[]>([]);
  ...
  const columns = [
    ...
    { 
      title: "Số tiền", 
      dataIndex: "Price",
      render: (a) =>  { return <p>{Intl.NumberFormat('ja-JP').format(a)}</p> } 
    },
    { 
      title: "Còn nợ", 
      dataIndex: "MoneyInDebt",
      render: (a) =>  { return <p className="font-weight-blue">{Intl.NumberFormat('ja-JP').format(a)}</p> } 
    },
...
const HistoryRefunds = (props) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (fnStatus) => {
        switch(fnStatus) {
          case "Chờ duyệt": return <span className="tag green">{fnStatus}</span>;
          case "Không duyệt": return <span className="tag red">{fnStatus}</span>;
          case "Đã duyệt": return <span className="tag yellow">{fnStatus}</span>;
          break;
        }
      },
    },
...
const InfoPaymentCard = (props) => {
  const id = props.id;
  const { showNoti } = useWrap();
  const [studentPriceAll, setStudentPriceAll] = useState<ICourseOfStudentPirce[]>([]);
  ...
  const getCourseOfStudentPriceAll = async () => {
    try {
      let res = await courseOfStudentPriceApi.getAll({selectAll: true, UserInformationID: id});
      if(res.status == 200) {
