...
function ExamReview() {
  ...
  const { packageID: packageID } = route.query;
    ...
    <div className="exam-review">
      <Card title={`${examInfo?.Code || "..."} - ${examInfo?.Name || "..."}`}>
        ...
        <Card.Grid className="exam-review-item span-2" hoverable={false}>
          ...
          <p className="desc">
            {examInfo?.NormalExercise >= 0 ? examInfo?.NormalExercise : "..."}
          </p>
        </Card.Grid>
        <Card.Grid className="exam-review-item span-2" hoverable={false}>
          <p className="title">Số câu khó</p>
          <p className="desc">
            {examInfo?.DifficultExercise >= 0
              ? examInfo?.DifficultExercise
              : "..."}
          </p>
        </Card.Grid>
        ...
        <Link
          href={{
            pathname: "/doing-test",
            query: { examID: ID, packageID: packageID },
          }}
        >
