...
const ListNewsFeed = (props) => {
    ...
    return (
      <>
        ...
        <Modal
          ...
          footer={null}
          onCancel={() => {
            setIsVisibleModal(false), resetOption();
          }}
        ...
        >
          <div className="container-fluid wrap-create-nf">
            <Form form={form} layout="vertical" onFinish={onSubmit}>
              ...
              <div className="row">
                <Form.Item name="Nội dung" initialValue={data.Content}>
                  <TextArea
                      ...
                      <div className="list-option">
                        <div className="item-option">
                          <Tooltip title="Thêm Ảnh/Video">
                            <div
                              className={
                                isOpenUploadFile ? "btn active" : "btn"
                              }
                            ...
                            <div className="item-option">
                              <Tooltip title="Chia sẻ vào trung tâm">
                                <div
                                  ...
                                  onClick={() => setChooseBranchFunc("team")}
                                >
                                  <GroupOutlined style={{ color: "#ffc107" }} />
                                </div>
                              </Tooltip>
                            ...
                            <div className="item-option">
                              <Tooltip title="Chia sẻ vào nhóm">
                                <div
                                  ...
                                  onClick={() => setChooseBranchFunc("group")}
                                >
                                  <TeamOutlined style={{ color: "#00afef" }} />
                                </div>
                              </Tooltip>
                          ...
                          {chooseBranch == "team" ? (
                            ...
                            >
                              <Select
                                ...
                                allowClear={true}
                                onChange={(value) =>
                                  setValue("GroupNewsFeedID", value)
                                }
                                disabled
                              >
                                {groupNewsFeed &&
                                  groupNewsFeed.map((item, index) => (
  ...
  const CommentAction = (props) => {
    ...
    return (
      <div className="info-current-user user-comment">
        <div className="avatar">
          <img
            src={dataUser?.Avatar ? dataUser.Avatar : "/images/user.jpg"}
            alt=""
          />
        </div>
  ...
  const ReplyComment = (props) => {
    ...
    const getCommentReply = async () => {
      try {
        let res = await newsFeedCommentReplyApi.getAll({
          selectAll: true,
        ...
        {commentReply?.map((item, index) => (
          <li key={index} className="item-comment">
            <div className="info-current-user">
              <div className="avatar">
                <img
                  src={item?.Avatar ? item.Avatar : "/images/user.jpg"}
                  alt=""
                />
              </div>
      ...
      <li className="item-comment">
        <div className="info-current-user">
          ...
          <div className="content-comment">
            <div className="box-comment">
              <p className="name-comment font-weight-black">
                {dataCommentIndex?.FullNameUnicode}
              </p>
            ...
            <span className="time-comment">
              {moment(dataCommentIndex?.CreatedOn).format("DD/MM/YYYY HH:mm")}
            </span>
            {addCommentAction && (
              <CommentAction
                reply={true}
            ...
            {dataCommentIndex.isReply && (
              <ReplyComment id={dataCommentIndex?.ID} />
            )}
          </div>
        </div>
      </li>
  ...
  const NewsFeed = (data) => {
    ...
    const checkedLike = async () => {
      try {
        let res = await newsFeedLikeApi.getAll({
          selectAll: true,
          NewsFeedID: data.data?.ID,
          UserInformationID: dataUser?.UserInformationID,
        });
        if (res.status == 200) {
    ...
    const getTotalComment = async () => {
      try {
        let res = await newsFeedCommentApi.getAll({
          selectAll: true,
          NewsFeedID: data.data?.ID,
        });
        if (res.status == 200) {
    ...
    return (
      <li className="item-nf">
        <div className="newsfeed">
          <div className="newsfeed-header">
            <div className="info-current-user">
              <div className="avatar">
                <img
                  src={
                    data.data?.Avatar ? data.data?.Avatar : "/images/user.jpg"
                  }
                  alt=""
                />
              </div>
              <div className="name-user">
                <div className="name">
                  <a data-text={data.data?.FullNameUnicode} onClick={_onSearch}>
                ...
                <span className="newsfeed-time">
                  {moment(data.data?.CreatedOn).format("DD/MM/YY HH:mm")}
                </span>
              </div>
            </div>
          ...
          {Object.keys(data.data?.NewsFeedFile.filter((item) => item.Type == 2))
            .length > 0 && (
            <div className="newsfeed-images">
              {Object.keys(
                data.data?.NewsFeedFile.filter((item) => item.Type == 2)
              ).length > 2 ? (
                <div className="more-than-3-images">
                  {data.data?.NewsFeedFile.filter(
                    (item, idx) => idx < 2 && item.Type == 2
                  ).map((item, index) => (
                    <Image
              ...
              <div style={{ display: "none" }}>
                ...
                >
                  {data.data?.NewsFeedFile.filter((item) => item.Type == 2).map(
                    (item, index) => (
          ...
          {Object.keys(data.data?.NewsFeedFile.filter((item) => item.Type == 3))
            .length > 0 && (
            <div className="newsfeed-audio">
              {data.data?.NewsFeedFile.filter((item) => item.Type == 3).map(
                (item, index) => (
          ...
          <div className="newsfeed-lightbox"></div>
          <div className="newsfeed-total">
            {totalLike > 0 && (
              <p>
                <ThumbsUp color="#0571e5" /> {totalLike}
              </p>
            )}
            {totalComment > 0 && (
              <p
